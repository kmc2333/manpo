...
class _ManpoKeiState extends State<ManpoKeiPage> {
  ...
  Future<void> start() async {
    ...
    setState(() {
      running = true;
      mHist.clear(); // グラフ履歴リセット
    });

    // JS から (ax, ay, az, t) を受け取って状態更新
  ...
  void _onMotion(num ax, num ay, num az, num t, int startMs) {
    ...
    // --- 歩数判定 ---
    _ema = 0.9 * _ema + 0.1 * mm; // 平滑化
    final diff = mm - _ema;

    // 閾値を下から上に跨いだら 1 歩（かつ連打防止）
          ...
          children: [
            ...
            Row(
              children: [
                ...
                SizedBox(
                  width: 52,
                  child: Text(threshold.toStringAsFixed(1)),
                ),
              ],
            ),
            const SizedBox(height: 8),
